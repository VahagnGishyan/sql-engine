...
namespace SQLEngine
{
    ...
    class IDataBase : public IDBComponent
    {
       ...
       public:
        virtual auto IsConnected() const -> const bool = 0;
    };
    ...
    def set_name(self, name):
        ...
        if self.path is not None:
            ...
            if os.path.exists(new_path):
                raise ValueError(
                    "A database with the same name exists in the parent
directory.") # Update the name and path self.path = new_path self.name = name
    ...
    def create(dbname, path=None, connect=False):
        ...
        if not utfile.is_path_exists(dbDataFile) or
utfile.is_file_empty_or_spaces(dbDataFile): utfile.mkdir(tblDir)

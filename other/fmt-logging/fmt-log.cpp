...
namespace SQLEngine::Logging
{
    ...
    FMTLogger::FMTLogger() : m_logfile{},
                             m_consoleMode{Mode::Default}
    {
        m_logfile.open(GetLogPath());
        if (!m_logfile)
        {
            throw std::runtime_error{fmt::format("Enable to open file: {}", GetLogPath())};
        }
    }
    ...
    {
        std::string path = "/home/vgishyan/sql-engine";
    ...
    auto ColorToFMTColor(const Color &color) -> fmt::color
    {
        ...
        case Color::Default:
        default:
            throw std::invalid_argument{"ColorToFMTColor: case default, invalid or unsupported color."};
        }
    }

    auto GetFMTTextStyle(const Mode &mode, const Color &color) -> std::optional<fmt::v10::text_style>
    {
        switch (mode)
        {
        case Mode::Info:
        ...
        default:
            throw std::invalid_argument{"GetFMTTextStyle: case default, invalid or unsupported mode."};
        }
    }

    void FMTLogger::PrintInConsole(const std::string &message, const Mode &mode, const Color &color)
    {
        if (GetMode() <= mode)
        {

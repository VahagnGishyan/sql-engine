...
class Database:
    ...
    def drop_table(self, table_name):
        # Find and remove the specified table from the database
        table_to_drop = next((table for table in self.tables if table.name == table_name), None)
        if table_to_drop:
            self.tables.remove(table_to_drop)
        else:
